...
static acl::string __keypre("bob_rankBattle");
...
static bool test_zrem(acl::redis_zset& redis, int n)
{
	...
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < 1000; j++)
		{
			member.format("user_%d", j);
			members.push_back(member);
		}

		redis.clear();
		int ret = redis.zrem(__keypre.c_str(), members);
		if (ret < 0)
		{
			printf("zrem error, key: %s\r\n", key.c_str());
//			return false;
...
static bool test_zrangebyscore(acl::redis_zset& redis)
{
	double min = 2, max = 10;
	...
	int ret = redis.zrangebyscore(__keypre.c_str(), -10000, 1000000, &result);
	if (ret < 0)
	{
		printf("zrangebyscore error, key: %s\r\n", __keypre.c_str());
		return false;
...
int main(int argc, char* argv[])
{
	...
	acl::string addr("https://bob-cosmos-kor.documents.azure.com:443"), cmd;
	cmd = "all";
	...
	client.set_password("XYOzP3Ej3nhn052d6E7W0HI1HDzZrtnAPoKxrWO59F7gzESmVcmXLJTGa59XvS5nMq6qWbVMP6rU3LAXT5GmDw==");
